<div class="row">
    <div class="col-lg-5">
        ...
        <h6 class="d-flex align-items-center mb-3" style="color:black;">Contribution Transaction History</h6>
    </div>
    <div class="col-lg-7">
...
<div class="table-responsive">
    <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
        <?php
            ...
            $array_count = 0;
            // Get details of approved contribution subscription to account number
            $sql = "SELECT subscription_id, amount, creation_date, staff_name FROM ckc_2021_contribution_subscription WHERE acc_number = '$acc_number' AND complete_status = '$incomplete'";
            ...
            if ($result = mysqli_query($link, $sql)) {
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        $transaction_amount_subscribed += $row['amount'];
                        $contribution_subscription_id[$array_count] = $row['subscription_id'];
            ...
            $a = 0;
            for ($i = 1; $i <= count($contribution_subscription_id); $i++) {

                $new_contribution_subscription_id = $contribution_subscription_id[$a];
                
                // Get details of subscription ID by account number
                $sql = "SELECT amount_due, voucher_date, staff_name, subscription_id FROM ckc_2021_contribution_deposit WHERE subscription_id = '$new_contribution_subscription_id'";
                ...
                if ($result = mysqli_query($link, $sql)) {
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            $transaction_history[$count_array_number][] = $newdata;
                            $transaction_amount_due += $row['amount_due'];
                            $count_array_number++;
            ...
            // Create new array with KEY VALUE pairs to store values gotten from contribution subscription and repayment
            ...
            foreach (array_keys($voucher_date_array) as $key => $val) {
                ...
                $transaction_subscription_id = $superhero[$val]["subscription_id"];
                
                echo "<tr>";
                    
                    echo "<td>" . $transaction_subscription_id . "</td>";
    ...
    <?php
        ...
        $transaction_balance = $transaction_amount_due - $transaction_amount_subscribed;
